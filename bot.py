...
# ==================== CONFIG ====================
BOT_TOKEN = "8520212996:AAFxALtBCRBt5gNi_4xnQBCbFt26B7MdKTw"
ADMIN_IDS = []
...
def create_cashfree_upi_qr(user_id, plan):
    """Create a Cashfree order and request a dynamic UPI QR.

    This intentionally does NOT auto-approve the bot payment. The QR is only
    used to collect the payment; the existing screenshot + admin approval flow
    remains the source of truth for access.
    """
    ...
    try:
        r = requests.post(f'{CASHFREE_BASE_URL}/pg/orders', headers=headers, json=payload, timeout=20)
        if r.status_code >= 300:
            return None, None, None, f'Cashfree order error {r.status_code}: {r.text[:300]}'
        order = r.json()
        session_id = order.get('payment_session_id')
        if not session_id:
            return None, None, None, 'Cashfree did not return payment_session_id.'

        qr_payload = {
            'payment_session_id': session_id,
            'payment_method': {'upi': {'channel': 'qrcode'}}
        }
        r2 = requests.post(f'{CASHFREE_BASE_URL}/pg/orders/sessions', headers=headers, json=qr_payload, timeout=20)
        if r2.status_code >= 300:
            return None, order_id, None, f'Cashfree QR error {r2.status_code}: {r2.text[:300]}'
        pay = r2.json()
        cf_payment_id = str(pay.get('cf_payment_id') or '')
        data = pay.get('data') or {}
        payload_data = data.get('payload') or {}
        qr_value = payload_data.get('qrcode') or pay.get('qrcode')
        if not qr_value:
            return None, order_id, cf_payment_id, 'Cashfree did not return a QR code. Ask Cashfree to enable S2S UPI QR for this account.'

        if qr_value.startswith('data:image') and ',' in qr_value:
            qr_value = qr_value.split(',', 1)[1]
        try:
            image = BytesIO(base64.b64decode(qr_value))
            image.seek(0)
        except Exception:
            # Some responses may return a UPI URI instead of base64 image.
            image = generate_upi_qr(qr_value, plan['price'], plan['name'])
        ...
        logger.error(f'Cashfree QR error: {e}')
